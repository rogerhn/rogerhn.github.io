...
function insertButtons(){
  ...
  var vidposter = document.querySelector('.vjs-poster');
  if(vidposter){
    ...
    console.log("Added download buttons!");
  }
  ...
  console.log("Added movname input");
}

function insertImage(){
  var vidposter = document.querySelector('.vjs-poster');
  if(vidposter){
    var style = vidposter.currentStyle || window.getComputedStyle(vidposter, false);
    ...
    var imgtoadd_down = document.createElement("button")
    imgtoadd_down.className ="img_added fa fa-download";
    imgtoadd_down.setAttribute('img_link', bi);
    imgtoadd_down.setAttribute('img_name', '');
    imgtoadd_down.appendChild(imagetoadd);
    var down_block = document.querySelector('.down_block');
    down_block.appendChild(imgtoadd_down);
    down_block.appendChild(imagetoadd);
    console.log("Added movie image!");
  }
}
...
function checktitle(){
  ...
  var mov_site = mov_category_block[0].title.replace(/ /g, "");

  var mov_category_formatted = '['+mov_site+']';

  var movtitle = document.getElementsByClassName('player-header__title');
  var mov_name = movtitle[0].textContent;

  var dateblock = document.getElementsByClassName('player-header__date');
  var dateblockinner = dateblock[0];
  var mov_date = dateblockinner.textContent;
  mov_date = mov_date.replace(/January/g, "01").replace(/February/g, "02").replace(/March/g, "03").replace(/April/g, "04").replace(/May/g, "05").replace(/June/g, "06").replace(/July/g, "07").replace(/August/g, "08").replace(/September/g, "09").replace(/October/g, "10").replace(/November/g, "11").replace(/December/g, "12").replace(/st,/g, "").replace(/nd,/g, "").replace(/rd,/g, "").replace(/th,/g, "").replace(/ /g, ".")
  ...
  document.querySelectorAll('.player-header__actors a').forEach(function (actors) {
    ...
    all_girls_list += names + ", ";
    mov_girls.push(names);

  });

  all_girls_list = all_girls_list.replace(/(.+), $/, '$1');
  var movname_final = mov_category_formatted + ' ' + all_girls_list + ' (' + mov_name + ' ' + mov_date + ')';
  ...
  if(vidposter){
    ...
    down_SD_button.addEventListener('click', function () {
      ...
      if (localStorage.rk_movs) {
        ...
        if(found === 0){
          storage.push({mov_id:mov_id,
                        mov_img:mov_img,
                        mov_link:mov_link,
                        mov_title:mov_title,
                        mov_site:mov_site,
                        mov_name:mov_name,
                        mov_ngirls:mov_girls,
                        mov_date:mov_date});
    ...
    remove_buttom.addEventListener('click', function () {
      var stored_movtitle = this.getAttribute("mov_id");
...
<p class="movs_page_text">Movies downloaded: ${storage_len}</p>
</div>
${mov_collection_list}
</div>`;
...
function movs_page_imgdown(){
  ...
  body.onkeydown = function(event) {
    ...
    if (key === 'x' || key === 'KeyX' || key === 88) {
      var imgToggler = document.querySelector('.toggleimg');
      imgToggler.classList.toggle('active');
      var using_nsfw = rk_settings.using_nsfw;
      var images = document.querySelectorAll('.slick-slide img,.player-video .video-js,article .card-thumb__img img,.picture-list a img,.mov_collection img.mov_col_img,.model-picture__thumb img,.gallery-thumbs .gallery-thumb__img,.timeline-caps__thumb img').forEach(function (image) {
        if (using_nsfw){
  ...
  document.addEventListener('click', function (e) {
    ...
    if ( e.target.classList.contains( 'img_added' ) ) {
      var img_link = e.target.getAttribute("img_link");
      var img_name = e.target.getAttribute("img_name");
      img_name = img_name.replace(/\(/g, "[").replace(/\)/g, "]")+ '.jpg';
      imageDownload(img_link, img_name)
